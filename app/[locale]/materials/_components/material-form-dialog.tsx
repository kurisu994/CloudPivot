...
const EMPTY_FORM: MaterialFormData = {
  id: null, code: "", name: "", material_type: "raw",
  category_id: null, spec: "", base_unit_id: null,
  aux_unit_id: null, conversion_rate: null,
  ref_cost_price: 0, sale_price: 0,
  safety_stock: 0, max_stock: 0,
  lot_tracking_mode: "none",
  texture: "", color: "", surface_craft: "",
...
export function MaterialFormDialog({
  ...
  useEffect(() => {
    ...
    if (materialId) {
      ...
      if (!isTauriEnv()) {
        // Mock 模式
        setForm({ ...EMPTY_FORM, id: materialId, code: "M-0001", name: "测试物料", material_type: "raw" });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[90vh] max-w-4xl flex-col overflow-hidden p-0 sm:max-w-4xl">
              ...
              <div className="grid grid-cols-4 gap-4">
                ...
                <Field data-invalid={!!errors.material_type || undefined}>
                  ...
                  <RadioGroup
                    ...
                    onValueChange={(v) => setField("material_type", v)}
                ...
                <Field data-invalid={!!errors.category_id || undefined}>
                  <FieldLabel>{t("form.category")} <span className="text-destructive">*</span></FieldLabel>
                  <Select
                ...
                <Field data-invalid={!!errors.base_unit_id || undefined}>
                  <FieldLabel>{t("form.baseUnit")} <span className="text-destructive">*</span></FieldLabel>
                  <Select
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <Field>
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setField("aux_unit_id", v === "none" ? null : parseInt(v))}
                    items={auxUnitItems}
                  >
                    <SelectTrigger>
                      <SelectValue />
                ...
                <Field>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-muted-foreground whitespace-nowrap">1辅 =</span>
                    <Input
                      id="conversion"
                      type="number"
                      step="0.01"
                    ...
                    />
                    <span className="text-sm text-muted-foreground whitespace-nowrap">主单位</span>
                  </div>
                </Field>
                ...
                <Field>
                  ...
                  <RadioGroup
                    value={form.lot_tracking_mode}
                    onValueChange={(v) => setField("lot_tracking_mode", v)}
                    className="flex gap-4 pt-1"
                  >
                    <label className="flex items-center gap-1.5 text-sm">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="relative">
                      <Input
                        ...
                        placeholder="0"
                      />
                      <span className="absolute top-2.5 right-3 text-xs text-muted-foreground">{t("form.labelL")}</span>
                    </div>
                    <div className="relative">
                      <Input
                        type="number"
                        ...
                        placeholder="0"
                      />
                      <span className="absolute top-2.5 right-3 text-xs text-muted-foreground">{t("form.labelW")}</span>
                    </div>
                    <div className="relative">
                      <Input
                        type="number"
                        ...
                        placeholder="0"
                      />
                      <span className="absolute top-2.5 right-3 text-xs text-muted-foreground">{t("form.labelH")}</span>
                    </div>
                  </div>
                </Field>
              </FieldGroup>
        ...
        <DialogFooter className="m-0 flex items-center justify-between rounded-b-xl border-t bg-slate-50/50 px-6 py-4 dark:bg-slate-900/50 sm:justify-between">
          <p className="text-xs text-muted-foreground">
            ℹ {t("form.requiredHint")}
          </p>
...
function SectionTitle({ title, accent }: { title: string; accent?: boolean }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={`text-sm font-bold ${accent ? "text-amber-600 dark:text-amber-400" : "text-primary"}`}>
        {title}
      </span>
    </div>
  );
