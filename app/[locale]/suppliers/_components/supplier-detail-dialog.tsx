...
import { CheckCircle2, CircleDollarSign, MapPin, Phone, TreePine, TrendingUp, User2, Wallet } from 'lucide-react'
              ...
              <div className="flex items-center justify-between mb-8">
                <div>
                  <div className="flex items-center gap-3">
                    <h2 className="text-2xl font-bold text-slate-900 dark:text-white">{detail.supplier.name}</h2>
                    <Badge
                      variant="outline"
                      className={
                    ...
                    >
                      <span className={`mr-1.5 h-1.5 w-1.5 rounded-full ${detail.supplier.isEnabled ? 'bg-emerald-500' : 'bg-slate-400'}`} />
              ...
              <div className="grid grid-cols-3 gap-6 rounded-2xl border border-slate-200/60 bg-slate-50/50 py-5 px-8 shadow-sm dark:border-slate-800 dark:bg-slate-900/30 mb-8">
                <div className="flex items-center gap-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-0.5 truncate font-semibold text-slate-900 dark:text-white">{detail.supplier.contactPerson || '—'}</div>
                  </div>
                </div>
                                ...
                                <TableRow key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/50">
                                  <TableCell>
                                    <div className="flex items-center gap-3 py-1">
                                      ...
                                      <div>
                                        <div className="font-semibold text-slate-900 dark:text-white">{item.materialName}</div>
                                      </div>
                                    </div>
                                  </TableCell>
                                  <TableCell>
                                  ...
                                  <TableCell className="text-right font-semibold text-slate-900 dark:text-white">
                                    {item.supplyPrice === null
                                      ? '—'
                                      : formatAmount(item.supplyPrice, item.currency).replace(item.currency, '').trim()}
                                  </TableCell>
                                </TableRow>
                              ))
                            )}
