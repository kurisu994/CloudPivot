...
import { PushProductionDialog } from './push-production-dialog'
...
export function SalesOrderDetailDialog({ orderId, onClose }: SalesOrderDetailDialogProps) {
  ...
  return (
    <>
      <Dialog open={orderId != null && !pushOpen} onOpenChange={open => !open && onClose()}>
      ...
      {/* 下推生产弹窗 */}
      <PushProductionDialog detail={pushOpen ? detail : null} onClose={() => setPushOpen(false)} />
    </>
  )
}
