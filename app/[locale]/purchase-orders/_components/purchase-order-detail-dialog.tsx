...
import type { PurchaseOrderDetail } from '@/lib/tauri'
import { getPurchaseOrderDetail } from '@/lib/tauri'
...
/** 采购单详情只读弹窗 */
export function PurchaseOrderDetailDialog({ orderId, onClose }: PurchaseOrderDetailDialogProps) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    void getPurchaseOrderDetail(orderId)
      .then(setDetail)
          ...
          {!loading && detail && (
            <div className="flex flex-col gap-6">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm lg:grid-cols-3">
                ...
                <div className="flex flex-col gap-1">
                  <span className="text-muted-foreground">{t('expectedDate')}</span>
                  <span>{detail.expectedDate ?? '-'}</span>
                </div>
                <div className="flex flex-col gap-1">
                ...
                <div className="flex flex-col gap-1">
                  <span className="text-muted-foreground">{t('payableAmount')}</span>
                  <span className="font-semibold text-blue-600 dark:text-blue-400">{formatAmount(detail.payableAmount, currency)}</span>
                </div>
                {detail.remark && (
