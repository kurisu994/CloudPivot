...
> **认证会话约定**：`users.session_version` 用于控制"记住我"本地会话失效；本地会话凭证存放于 `~/.cloudpivot/data/auth_session.json`，不在数据库中保存明文密码或长期 token。
...
> v1.0 最简版：含工单头、领料物料明细和完工入库记录，不含工序、报工、排产。  
> **注意**：实际实现表名为 `production_orders`（早期设计文档中为 `work_orders`，已废弃）。
...
#### production_order_materials — 工单领料/退料物料明细
...
| `production_orders`          | `status` | `draft` / `picking` / `producing` / `completed` / `cancelled`     | 草稿 / 领料中 / 生产中 / 已完工 / 已取消   |
