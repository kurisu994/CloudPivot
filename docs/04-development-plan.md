...
- [x] "记住我"仅通过系统钥匙串/凭据管理器保存本地会话，不落盘明文密码（keyring crate，macOS/Windows/Linux 原生支持）
...
| 2.16  | **物料 Excel 导入导出** — 模板、字段映射、预览校验                    | 1.5d | P1     | 2.1     | ✅ 已完成（模板下载 + 预览校验 + 导入/导出 IPC + 核心字段引用保护）          |
...
- [x] 物料可通过 Excel 模板导入导出，字段映射和校验提示可用
...
| 里程碑                   | 预计完成        | 核心交付                                                                                                                                                                 | 状态              |
| ------------------------ | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------- |
| M1：脚手架就绪           | 第 4 周         | 项目骨架 + 数据库 + 布局 + **i18n + 多币种/舍入规则基础 + 配置模型 + 本地会话认证（钥匙串）**                                                                            | 🔶 进行中（~90%，钥匙串已完成，Repository trait/汇率 CRUD/CI 待完成） |
