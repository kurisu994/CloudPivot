...
- [x] "记住我"仅保存本地会话凭证，不保存明文密码；Tauri 环境写入应用数据目录认证文件，改密后通过 `session_version` 失效
...
| 3.20a        | 自由出入库 — Rust 层 + 前端页面（不关联采购/销售单，直接生成其他入库/其他出库流水）                          | 1d   | P1     | 3.13       | ✅ 已完成（1 个 IPC 命令 + 独立表单页） |
...
| “记住我”本地会话安全         | 本地凭据泄露             | 中   | 本地会话写入应用数据目录认证文件，Unix 收敛到 0600 权限，改密或重置密码后通过 `session_version` 失效           |
...
| 认证       | 密码 bcrypt 哈希存储；“记住我”使用应用数据目录认证文件保存本地会话凭证；单据统一使用 `*_user_id` + 名称快照记录操作者 |
