...
> **📊 进度**（截至 2026-04-22）：**9/16 项完成**（约 56%）。销售单 Rust CRUD（6 个 IPC 命令）+ 列表页 + 编辑页（行折扣 + 整单折扣率 + 信用额度提醒）已完成。销售出库 Rust 层（3 个 IPC 命令，含 FIFO 批次分配 + 双成本快照固化 + 费用分摊 + 应收生成）+ 前端（列表页 + 执行页）已完成。销售退货 Rust + 前端（3 个 IPC 命令 + 列表页 + 执行页，含成本回调 + 应收冲减）已完成。定制单管理 Rust（9 个 IPC 命令）+ 前端（列表页 + 详情/编辑页）已完成。库存盘点和库存调拨已完成。**剩余**：生产工单（3.24-3.27）。
...
| M3：销售+定制单+库存高级 | 第 17.5-18.5 周 | 销售/**单头仓库规则**/库存高级(盘点+调拨)/预警/**批次追溯**/**原材料预留 + 成品预留链路**/**期初库存导入**/**操作日志**/**定制单管理**/**生产工单(承接预留+领料+完工)**/ | 🔶 进行中（销售/库存/定制单已完成，工单待开发） |
