...
| #     | 任务                                                                                                                                                                                                 | 预估 | 产出物                                   | 状态                                                                                 |
| ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | ---------------------------------------- | ------------------------------------------------------------------------------------ |
| 1.0   | **技术验证（扩展）** — ① Next.js 16 SSG + `[locale]` 路由 + Tauri 文件协议端到端验证 ② @react-pdf/renderer 越南语/中文字体嵌入 POC ③ Tauri 本地会话文件持久化验证 ④ 万级数据 IPC 传输性能基准测试 | 1d   | 验证报告 + POC 代码                      | 🔶 部分完成（①已验证）                                                               |
...
| 3.20b        | 批量出入库单 — 升级为批量草稿单据（同方向/同仓库/最多 100 行明细，固定手工业务类型，草稿暂存 + 整单原子过账 + FIFO/移动加权成本 + 高风险二次确认；流水页区分手工与正式来源）。新增 `manual_stock_movement.rs`（5 个 IPC）+ 表 `manual_stock_movements`/`manual_stock_movement_items`（迁移 005）；废弃旧 `create_manual_stock_movement` | 3d | P1 | 3.20a | ✅ 已完成（详见 `docs/superpowers/specs/2026-05-27-batch-manual-stock-movements-design.md`） |
...
- [ ] 盘点盈亏审核后正确调整库存（含盈亏金额）
- [ ] 调拨同时更新两个仓库库存
- [ ] 自由入库/出库不生成应收应付，正确更新库存、批次和 `other_in` / `other_out` 流水
...
| 5.8            | 数据备份与恢复 + 自动备份（手动备份/恢复已完成，自动备份待实现）                       | 2d   | P0     | —          |
...
| 里程碑                   | 预计完成        | 核心交付                                                                                                                                                                 | 状态              |
| ------------------------ | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------- |
| M1：脚手架就绪           | 第 4 周         | 项目骨架 + 数据库 + 布局 + **i18n + 多币种/舍入规则基础 + 配置模型 + 本地会话认证（认证会话文件）**                                                                      | 🔶 进行中（~90%，认证持久化已完成，Repository trait/汇率 CRUD/CI 待完成） |
| M2：基础数据完成         | 第 7.5 周       | 物料/分类/供应商(**含物料关联**)/客户/仓库/**默认仓映射**/BOM(**含物料反查**)/**编码规则/库存规则基础设置**                                                              | 🔶 进行中（~75%，物料导入导出待完成） |
| M2.5：采购+库存基础      | 第 11.5 周      | 采购全流程 + 库存基础能力联调通过 + **操作日志采集与查询**                                                                                                               | 🔶 进行中（操作日志已完成，库存查询/出入库流水/期初导入待完成） |
| M3：销售+定制单+库存高级 | 第 17.5-18.5 周 | 销售/**单头仓库规则**/库存高级(盘点+调拨)/预警/**批次追溯（FIFO 出库 + lot 级预留分配）**/**原材料预留 + 成品预留链路**/**操作日志**/**定制单管理**/**生产工单(承接预留+领料+完工)**/ | 🔶 进行中（销售/库存/定制单/工单/批次追溯均已完成，待集成测试） |
| M4：财务报表 + 补货      | 第 21-22 周     | 应收应付/**智能补货**/报表/**标准/实际毛利**/看板（全部真实数据）                                                                                                        | 🔶 进行中（财务+补货+报表+看板已完成，剩余毛利分析增强） |
