...
> **菜单开放说明**：当前 `config/nav.ts` 仅开放首页、物料/分类/单位、库存查询/自由出入库/出入库流水/库存盘点、智能补货、库存报表、操作日志和外观设置；其余已实现模块暂时隐藏入口，但保留路由与后端能力。
...
| ~~P2~~ | ~~多终端访问~~        | ✅ 已完成基础能力：基于 PostgreSQL 支持局域网多终端共享数据；多帐号权限仍属 v2.0           |
