...
从 v1.0 单管理员模式升级为多账号 + 角色权限体系。基于需求规格 §3.9.6 中的 v2.0 演进方向设计。

## 设计决策

### 权限粒度

方案采用 **模块 × 操作** 二维权限矩阵（而非页面级或按钮级），与需求规格中的权限矩阵对齐。内置三个系统角色：**管理员 (admin) / 操作员 (operator) / 查看者 (viewer)**，v1 先不开放自定义角色。
