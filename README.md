...
| 3   | [界面原型设计](docs/03-ui-prototype.md)   | 整体布局、**32 个页面** wireframe、交互规范、全量页面地图                 |
| 4   | [开发计划](docs/04-development-plan.md)   | 甘特图、**5 个开发阶段**任务清单、技术风险                                |
| 5   | [页面开发进度清单](docs/05-page-todo-list.md) | App Router 页面开发与对接状态（32/32 已完成）            |
...
app/                        # Next.js App Router（Tauri 生产构建使用 SSG）
  ...
  [locale]/                 # i18n 路由（next-intl）
    ...
    reports/                # 报表中心（采购/销售/库存报表）
    settings/               # 系统设置（8 个子页面）
    setup-wizard/           # 首次使用向导
components/
...
src-tauri/                  # Rust 后端
  Cargo.toml                # tauri 2.10, sqlx(sqlite), bcrypt, chrono, uuid, thiserror, keyring
  src/
