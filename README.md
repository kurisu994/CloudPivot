...
| 5   | [用户管理设计](docs/07-user-management-design.md) | 多账号/权限矩阵设计草案（当前代码仍为单管理员模式）                   |
...
app/                        # Next.js App Router（Tauri 生产构建使用 SSG）
  ...
  [locale]/                 # i18n 路由（next-intl）
    ...
    reports/                # 报表中心
      ...
      purchase/             # 采购报表
      sales/                # 销售报表
    settings/               # 系统设置（9 个页面路由，其中 user-management 当前为提示页）
    setup-wizard/           # 首次使用向导
components/
...
- [x] 用户管理占位页 — 保留 `/settings/user-management` 路由，用于明确当前仍为单管理员模式
