...
**颗粒度与表述要求**：
- 颗粒度控制在「改动大项」级别。一条 entry = 一个用户可感知的能力或行为变化；同一大项下的多处小改动合并描述，不要逐处罗列。
- 不写具体数值变化（像素、行高、间距、按钮个数、表数量、命令数量、字段数量、迁移版本号等）。需要描述强度时用「明显减小 / 显著加快 / 不再出现」等定性词。
- 不使用设计或架构内部术语（矩阵、二维、命名空间、模块 × 操作、provider、hook、CSS 变量、迁移脚本、ON CONFLICT 等）。同义功能描述替换。
- 不引用本仓库内部的迁移编号、commit hash、PR 号。
- 例：「开启后表头高度由 40px 缩至 32px、单元格上下内距由 8px 缩至 4px」❌；「开启后整体行高与单元格内距明显减小」✅
- 例：「按「模块 × 操作」二维权限矩阵控制能力范围」❌；「按角色控制可见菜单与可用操作」✅
...
| **自由出入库** | ✅ | 5 命令 | 列表+草稿编辑页(多行明细重构为 shadcn Table+明细合并+风控二次确认+超仓库存预检提示弹窗)+原子过账(FIFO批次/移动加权成本)；按 `lot_tracking_mode` 区分批次/非批次物料；废弃旧单物料即时过账 `create_manual_stock_movement` |
