...
| 批量出入库 | `manual_stock_movement.rs` | 5 | 列表/详情/保存草稿/确认过账(原子+FIFO+风控)/删除草稿 |
...
| **批量出入库** | ✅ | 5 命令 | 列表+草稿编辑页(多行明细+合并+风控二次确认)+原子过账(FIFO批次/移动加权成本)；按 `lot_tracking_mode` 区分批次/非批次物料；废弃旧单物料即时过账 `create_manual_stock_movement` |
| **出入库流水** | ✅ | 1 命令 | 列表(多维筛选+日期范围+变动类型Badge+来源/业务类型筛选，区分手工批量单与正式单据) |
