# 更新日志 (Changelog)

本文件记录 **云枢 (CloudPivot IMS)** 各版本的主要变更。

格式遵循 [Keep a Changelog](https://keepachangelog.com/zh-CN/1.1.0/) 规范，版本号遵循 [语义化版本](https://semver.org/lang/zh-CN/)。

---

## [Unreleased]

### 修复

- **隐藏登录页初始账号密码提示**：将登录页底部提示框（"提示：初始账号 admin / 初始密码 admin123，首次登录后需立即修改密码。"）用 JSX 注释包裹隐藏，避免默认凭证信息直接暴露在生产环境中。翻译键 `login.hint` 仍保留在 messages 中，后续如需恢复可直接取消注释。

---

## [0.1.9] — 2026-05-27

### 新增

- **退出登录入口**：顶栏右上角用户区改为下拉菜单，展示当前登录用户（昵称与角色）并提供「退出登录」操作，调用既有 `logout()` 清除会话并跳转登录页。此前该处为硬编码的「Admin User」死按钮，缺少任何退出入口。
- **自由出入库升级为批量草稿单据**：原单物料即时记账升级为批量草稿单据系统。新增表 `manual_stock_movements` / `manual_stock_movement_items`（迁移 `005`）与模块 `manual_stock_movement.rs`（5 个 IPC：列表/详情/保存草稿/确认过账/删除草稿），支持草稿暂存、多行明细与重复合并、12 种固定手工业务类型、整单原子确认过账（移动加权平均成本、按物料 `lot_tracking_mode` 决定 FIFO 批次扣减、非批次物料仅扣主库存）及数量/金额双阈值高风险二次确认。前端为单据列表 + 草稿编辑页（BusinessListTable 骨架），三语补全 12 类业务类型文案。

### 变更

- **系统副标题去地域化**：登录页、启动页等处副标题由「越南家具工厂进销存管理系统」改为「一体化进销存管理系统」（zh/en/vi 三语同步），不再绑定具体地域。
- **出入库流水来源区分与统计口径边界**：流水页新增「来源」列及来源类型、业务类型筛选，用以区分同名的自由出入库单与正式单据。库存事务写入策略：手工「采购入库」复用 `transaction_type='purchase_in'`、手工「生产领料」复用 `production_out`（使其自动纳入消耗分析与补货建议），其余手工类型写 `other_in`/`other_out`；所有手工流水统一 `source_type='manual_stock_movement'` 并通过 `source_id` 回链单据头。采购报表、供应商排行、应付账款、工单成本均按各自来源表/`source_type` 过滤，手工单不污染正式统计口径。`save`/`confirm`/`delete` 命令统一 `is_authenticated` 鉴权，确认命令在后端二次计算风控阈值，未携带确认标记时拒绝高风险过账。
- **盘点创建与录入流程**：盘点单创建时将范围内系统库存预填为实盘数量，操作员只需修改差异行；创建弹窗增加明细生成中的加载反馈，避免大范围盘点等待期间重复提交。
- **自由出入库命名回归**：出入库流水来源列与操作日志中手工单的显示名由「批量出入库单」改回「自由出入库（单）」（zh/en/vi 三语同步）；内部表与模块 `manual_stock_movements` 命名不变。
- **库存报表暂时隐藏金额列**：收发存 / 库龄表的金额列、顶部金额 KPI 卡片与导出 Excel 金额列暂时隐藏，由 `SHOW_AMOUNT_COLUMNS` 开关统一控制，数据口径可靠后翻开即可整体恢复。
- **全局表格分页器统一升级**：统一分页器视觉与默认每页条数；移除冗余的「回到首页」按钮（点页码「1」即可回首页）；精简操作日志底部分页统计文字。
- **系统设置页与顶栏微调**：系统设置页去除两侧最大宽度限制、内容铺满整屏；移除顶栏未启用的通知铃铛占位按钮。

### 修复

- **退出登录未记录操作日志**：此前 `logout()` 仅清除前端会话并跳转登录页，未触达任何后端写日志路径，导致退出动作在操作日志中缺失。新增后端 `logout` 命令与 `auth::logout`，写入 `action=logout` 日志，操作者身份取自 `CurrentUser`（须在清除会话前记录）；前端登出在清会话前调用该命令，失败不阻塞登出；操作日志页与 zh/en/vi 文案同步新增 logout 动作。
- **数据库时间戳时区漂移**：所有 `TIMESTAMP` 列经 `NOW()` 写入时按会话时区折算，此前未显式设置会话时区，导致存储时间随数据库宿主机本地时区变化（表现为被写成 CST）。连接池新增 `after_connect` 钩子，每条连接执行一次 `SET TIME ZONE 'Asia/Ho_Chi_Minh'`（UTC+7），确保写入时区稳定且与宿主机无关。
- **盘点明细创建耗时与 SQL 拼接**：物料级、批次级快照由逐行写入改为参数化集合插入，创建耗时不再随明细条数增加数据库往返，并移除批次范围查询中对仓库/分类 ID 的 SQL 字符串拼接。
- **自由出入库出库流水方向错误**：批量出库确认时库存流水数量未取负，导致「生产领料」等出库记录在出入库流水中被显示为入库（+）方向。已改为与销售出库（`sales_out`）、生产领料（`production_out`）一致的负数记账约定（`-deduct_qty` / `-item.quantity`）；修复后新流水方向正确，已写入的历史记录需一次性符号订正（`UPDATE inventory_transactions SET quantity = -quantity WHERE source_type='manual_stock_movement' AND transaction_type IN ('production_out','other_out') AND quantity > 0`）。
- **多处缺失 i18n 文案补齐**：补齐自由出入库列表合计（`manualStockMovements.totalItems`）、`common` 的单位 / 页码 / 上一页 / 下一页、客户删除失败提示等缺失键，并修正操作日志动作映射缺失（`confirm` / `save` / `delete_batch_movement`）。

---

## [0.1.8] — 2026-05-25

### 变更

- **主窗口默认最大化**：应用启动时窗口自动最大化填满屏幕工作区，保留标题栏并可还原至 1440×900；macOS/Windows/Linux 三平台行为一致。
- **菜单阶段性开放**：侧边栏当前仅开放首页看板、物料/分类/单位管理、库存查询/自由出入库/出入库流水/库存盘点、智能补货、操作日志、外观设置；BOM、采购、销售、定制单、生产工单、财务（应付/应收）、报表中心及供应商/客户/仓库/库存调拨/其余系统设置项用块注释暂时隐藏，后续分批开放。隐藏项仅移除侧边栏入口，对应页面路由与后端命令保留。

### 修复

- **Critical/High 级别安全与并发缺陷**：
  - SQL 注入防护：操作日志、盘点单、单号生成等模块改用 QueryBuilder 参数化查询，消除字符串拼接风险
  - PostgreSQL 方言兼容：`MAX(0, expr)` 统一改为 `GREATEST(0, expr)`
  - 并发覆盖保护：库存 SELECT 添加 `FOR UPDATE` 行锁，批次预留增加数量约束校验
  - IPC 鉴权增强：`CurrentUser` 增加 `is_authenticated` 校验，12 个写命令添加 `require_auth` 守卫
  - 密码修改安全：后端增加旧密码校验，前端改密页同步添加旧密码输入框
  - 迁移原子性：每条迁移在独立事务内执行，补齐 003/004 PostgreSQL 迁移脚本
  - BOM 删除保护：修正被引用字段判断条件
  - 备份恢复安全：改为事务内执行并添加鉴权校验
- **构建环境区分**：`next.config.ts` 仅在 Tauri `pnpm build` 生命周期启用静态导出，开发模式保持 Next.js 服务器，避免非 Tauri 环境下构建失败
- **i18n 翻译补全**：操作日志页面补全 zh/en/vi 三语下缺失的 `loading` 与 `noData` 翻译键
- **WebView 打印权限缺失**：补全 `core:webview:allow-print` 权限，使单据打印时 `window.print()` 能在 Tauri WebView 中正常唤起打印对话框，此前因缺少权限导致打印无响应。
- **应付/应收 KPI 货币单位错误**：应付账款与应收账款页面的 KPI 卡片（应付/应收总额、已付/已收、部分付款、逾期）金额硬编码按 VND 格式化，与系统基准币种不符，统一改为基准币种 USD。
- **多处硬编码中文文本**：清理 BOM 编辑/反查、定制单详情、物料表单、企业信息与打印设置页中的中文硬编码，改为 `t()` 国际化调用，并补全 zh/en/vi 三语对应翻译键。
- **物料分类筛选选父分类无结果**：物料挂在子分类下，而 `get_materials` 按 `category_id` 精确匹配，选父分类时查不到任何物料。改为借助 `categories.path` 前缀匹配按子树查询，选父分类可列出其全部子孙分类下的物料，选叶子分类时仍等价于精确匹配。
- **报表与库存查询的 PostgreSQL 类型不匹配**：
  - 滞销预警、库存账龄、库存批次明细的在库/滞销天数列由日期相减得 `INT4`，与 Rust `i64` 字段不兼容导致解码失败，统一补 `::BIGINT` 转换。
  - 库存趋势、滞销月均出库的 `transaction_date`（`TEXT` 列）直接与时间戳比较，触发 `operator does not exist: text >= timestamp`，补 `::DATE` 转换后再比较。

---

## [0.1.7] — 2026-05-19

### 变更

- **认证会话持久化策略降级**：Tauri 环境下“记住我”会话从系统钥匙串改为应用数据目录 `auth_session.json` 文件持久化，前端 API 语义同步调整为 `AuthSession`，保留旧 `AuthKeychain` 导出兼容现有调用。
- **项目文档同步认证持久化语义**：README、AGENTS 和需求/设计/开发文档统一改为“认证会话文件持久化”，移除已不适用的系统钥匙串/keyring/凭据管理器描述。

### 修复

- **认证会话文件落盘可靠性**：后端认证会话写入改为临时文件替换，Unix 平台限制文件权限为 `0600`，读取和清理对文件不存在保持幂等，并补充单元测试覆盖。
- **Cargo.toml 尾部空白**：清理移除 `keyring` 依赖后遗留的文件末尾空白行。

---

## [0.1.6] — 2026-05-19

### 修复

- **新增物料编码未自动生成**：补齐 `generate_material_code` IPC，按系统配置的物料编码前缀、起始流水号和流水位数生成编码；新增保存时若编码为空由后端兜底生成，编辑时禁止保存空编码。
- **自由出入库物料选择显示前导分隔符**：历史空编码物料在下拉和预览中不再显示 `- 物料名`，改为仅显示物料名称。
- **库存详情弹窗尺寸过小**：库存查询详情 Dialog 调整为更宽更高的明细视图，改善分仓、批次和近期流水表格阅读体验。
- **出入库流水长单号展示不全**：加宽流水号与关联单据列，并同步增大表格最小宽度，避免长单号溢出到相邻列。
- **物料管理字段命名不一致**：前端 `MaterialFormData` 和 `MaterialItem` 混用 snake_case 与 camelCase，与后端 `serde(rename_all = "camelCase")` 不匹配，导致保存物料报 `missing field` 错误、列表进价售价显示为空。统一为 camelCase。
- **物料价格存储错误**：表单直接存储用户输入值而未做币种精度转换（USD 以分为单位），输入 100 元实际存为 100 分。新增 `toStorageAmount`/`toDisplayAmount` 转换，货币符号改为从系统本位币配置动态读取。
- **物料 is_enabled 类型错误**：PostgreSQL `BOOLEAN` 列不接受整数 `1`/`0`（SQLite 遗留），INSERT 和 UPDATE 改为绑定原生布尔值 `TRUE`/`FALSE`。
- **物料分类选择器平铺无层级**：后端 `get_categories` 新增返回 `parent_id` 和 `level` 字段，前端按深度优先排序并加缩进展示层级。
- **物料弹窗尺寸过小**：`DialogContent` 从 `max-w-4xl` 调大 20% 至 `sm:max-w-[67rem]`。
- **报表 SQL 类型不兼容**：`reports.rs` 中 `SUM(bigint)` 返回 `NUMERIC`，sqlx 用 `i64` 解码失败。在 3 处 SQL 的 `SUM(...)` 外层包裹 `CAST(COALESCE(..., 0) AS BIGINT)` 强制转回 `BIGINT`。
- **盘点盈亏金额显示放大 100 倍**：盘点编辑页 `diffAmount` 直接 `.toFixed(2)` 展示数据库分值，未做分→元转换。改用 `formatAmount()` 正确格式化。

- **分类管理删除无效果**：Tauri 2 WebView 中 `window.confirm()` 不弹出原生对话框直接返回 false，改用 shadcn Dialog 组件实现确认弹窗。
- **分类管理创建/更新/排序失败**：`category.rs` 中 `SELECT level` 查询用 `i64` 解码 PostgreSQL `INTEGER`（INT4）列，sqlx 类型检查报错。三处统一改为 `i32`。
- **上级分类选择器层级错乱**：后端返回扁平列表按 `sort_order` 排序，子节点未紧跟父节点，缩进显示错位。新增深度优先遍历排序后再渲染下拉选项。

- **财务模块翻译缺失**：`finance.json` 缺少顶层 `"finance"` 命名空间包裹键，导致 `useTranslations('finance')` 无法解析翻译内容。三语（zh/en/vi）统一补齐命名空间结构。

- **记住我功能失效**：路由守卫缺少"已登录用户在登录页时重定向到首页"的规则，导致应用启动恢复会话后仍停留在登录页。新增第 4 条守卫规则及 `isPendingRedirect` 覆盖。
- **切换语言后跳转登录页**：`AuthProvider` 位于 `[locale]` layout 中，locale 变化导致组件重新挂载、state 重置为未登录态，路由守卫误判重定向。引入模块级缓存（`cachedUser` / `authInitialized`），locale 切换时同步恢复认证状态，跳过重复异步验证。
- **输入框首字母自动大写**：macOS WebView 默认对 input/textarea 启用 autocapitalize，在 ThemeProvider 中通过 MutationObserver 全局禁用 autocapitalize、autocorrect 和 spellcheck。

- **分类管理新增/更新无法挂到父级**：分类前端字段命名与后端 `serde(rename_all = "camelCase")` 不一致，`parent_id` 等 snake_case 字段无法被反序列化，导致新建分类总是落到顶级节点。统一 `lib/tauri/category.ts` 与分类页面字段为 camelCase，库存报表筛选中残留的 `parent_id` 引用一并修正。

### 重构

- **全站替换 `window.confirm` 为 `ConfirmDialog` 组件**：Tauri WebView 环境下原生 `window.confirm()` 被静默拦截导致删除/审核等操作无响应。新增 `components/common/confirm-dialog.tsx` 统一确认弹窗组件，覆盖 18 个业务页面（基础管理 5 + 业务列表 5 + 执行页 6 + 定制单详情 1 + 数据管理 1），采用 `pendingAction` 联合类型或 `showConfirm` 布尔状态驱动模式，支持 `destructive` 危险操作样式。

---

## [0.1.5] — 2026-05-13

### 重构

- **数据库迁移 SQLite → PostgreSQL**：将嵌入式 SQLite 替换为远程 PostgreSQL，支持多终端共享数据和集中式数据安全。
  - Cargo.toml 依赖从 `sqlx/sqlite` 切换为 `sqlx/postgres`，新增 `dotenvy` 用于编译时加载环境变量。
  - `build.rs` 支持从 `.env`（本地开发）或环境变量（CI 打包）注入 `DATABASE_URL`，编译时写入二进制。
  - `db/mod.rs` 重写为 `PgPool` 连接逻辑，移除 SQLite PRAGMA 和文件路径依赖。
  - `migration.rs` 适配 PostgreSQL，迁移脚本从 `migrations/sqlite/` 迁移到 `migrations/postgres/`。
  - 全部 18 个 Rust 源文件中的 `SqlitePool` → `PgPool`、`QueryBuilder<Sqlite>` → `QueryBuilder<Postgres>`、`SqliteConnection` → `PgConnection`。
  - 分页参数类型从 `u32` 改为 `i32`（PostgreSQL 不支持无符号整数）。
  - CI release workflow 新增 `DATABASE_URL` secret 注入。

- **采购/销售共享抽象**：创建 `order_shared.rs` 模块，提取两个模块中 6 类重复模式（列表查询构建器、编号生成、审核/作废/删除、状态更新、费用分摊、完成检查），消除约 516 行重复代码。

### 新增

- **统一 Loading 状态设计系统**：创建 `components/common/loading.tsx`，提供 6 种场景化 Loading 组件（PageLoading、CardLoading、InlineLoading、FormLoading、MetricCardLoading、ChartLoading），统一全应用加载态视觉风格。
- **Dashboard 快捷操作按钮功能**：4 个快捷按钮（新建采购单、新建销售单、采购入库、销售出库）添加路由导航，点击直接跳转对应页面；采购单/销售单支持 `?action=new` URL 参数自动进入新建模式。

### 新增

- **自由出入库（全栈）**：新增 `/manual-stock-movements` 页面和 `create_manual_stock_movement` IPC 命令，方便未启用采购单/销售单流程时直接登记其他入库或其他出库。
  - **自由入库**：选择物料、仓库、日期、数量和 USD 单位成本后即时过账，更新移动加权平均成本；批次追踪物料自动生成或使用手填批次。
  - **自由出库**：校验库存不足，批次追踪物料按 FIFO 扣减可用批次；仅生成 `other_out` 库存流水，不生成应收应付。
  - **流水与审计**：自由出入库统一生成 `FM-YYYYMMDD-XXX` 单号，写入库存流水和操作日志；出入库流水筛选补齐生产出入库、其他入库、其他出库类型。
  - 国际化：新增自由出入库中/越/英三语文案，并在库存导航中加入独立入口。
- **库存报表模块（全栈）**：新增 `/reports/inventory` 页面与 4 个报表 IPC 命令，覆盖收发存汇总、批次库龄分析、滞销预警和库存趋势数据。
  - **收发存汇总**：支持日期范围、仓库、分类、物料类型和关键词筛选，展示期初/入库/出库/结存数量与 USD 基准金额 KPI。
  - **批次库龄分析**：基于启用批次追踪的库存批次计算在库天数，支持仓库/分类/库龄区间/关键词筛选，并提供库龄分布饼图。
  - **滞销预警**：按自定义未出库天数阈值筛选当前有库存物料，展示最后出库日、未出库天数和近 90 天月均出库量。
  - **Excel 导出**：当前报表 Tab 按生效筛选条件拉取全量数据并导出 `.xlsx` 文件。
  - 国际化：新增 `reports` 命名空间，覆盖中/越/英三语。
- **采购/销售报表模块（全栈）**：新增 `/reports/purchase` 与 `/reports/sales` 页面，并扩展 6 个报表 IPC 命令。
  - **采购报表**：按已确认入库单统计采购汇总趋势、供应商排行、物料采购明细，支持日期、供应商、仓库和关键词筛选。
  - **销售报表**：按已确认出库单统计销售汇总趋势、客户排行、产品销售明细，支持日期、客户、仓库和关键词筛选。
  - **共用报表体验**：KPI 卡片、Recharts 趋势图、业务表格分页、全量 Excel 导出和中/越/英三语文案复用同一套页面骨架。

### 变更

- **自由出入库页面体验**：将单卡片表单调整为“主表单 + 过账预览”双栏工作台，右侧实时展示变动类型、物料、仓库、日期、数量、单位成本和必填状态；宽屏减少页面留白，窄屏自动堆叠保持可操作性。
- **前端依赖**：新增 `xlsx` 用于报表 Excel 导出。

### 修复

- **Excel 模板多语言列头**：物料导入导出和期初库存导入模板改为按当前语言生成列头，避免英文/越南语界面下载或导出仍显示中文字段名。
- **Tauri 2 运行环境识别**：修复 `just dev` 下前端误判为非 Tauri 环境的问题，避免自由出入库等写操作落入 mock 分支而没有真实写入库存和流水。
- **库存 IPC 参数映射**：补齐库存查询、出入库流水、盘点和调拨相关请求参数的 camelCase 反序列化支持，确保前端筛选条件能被 Rust 后端正确接收。
- **首次改密结果反馈**：改密成功后即使刷新用户信息短暂失败，也会使用本地会话状态完成跳转，避免后端已成功修改密码但界面提示失败。

---

## [0.1.4] — 2026-04-24

### 新增

- **库存管理模块（全栈）**：实现完整的库存查询 → 出入库流水 → 库存盘点 → 库存调拨全流程
  - **库存查询**：后端 2 个 IPC 命令（列表筛选分页 + 详情），列表支持关键词/仓库/分类/预警状态四维筛选，自动计算库存金额和可用量（总量 - 预留量），预警状态（low/high/normal）高亮显示；详情弹窗含分仓汇总、批次明细（含库龄天数）、近 10 笔流水三 Tab 视图
  - **出入库流水**：后端 1 个 IPC 命令（流水列表查询），支持关键词/仓库/变动类型/日期范围多维筛选分页，变动类型覆盖采购入库/销售出库/采购退货/销售退货/盘盈/盘亏/调拨入/调拨出
  - **库存盘点**：后端 5 个 IPC 命令（列表/详情/创建/更新实盘/审核确认），创建时自动快照系统库存，支持整仓或按分类盘点，审核确认时在事务中生成盘盈/盘亏流水并调整库存（复用 `inventory_ops` 的 `increase_inventory`/`decrease_inventory`）
  - **库存调拨**：后端 5 个 IPC 命令（列表/详情/保存/确认/删除），草稿态支持编辑和删除，确认调拨时在事务中完成：源仓减库存 → 目标仓加库存 → 批次搬仓 → 生成调拨出/入双向流水
  - **前端库存查询页面**：列表页（四维筛选 + 预警行高亮 + 库存金额显示）+ 详情 Dialog（分仓汇总 / 批次明细 / 近期流水三 Tab）
  - **前端出入库流水页面**：列表页（多维筛选 + 日期范围 + 变动类型 Badge 颜色区分 + 增减量红绿显示）
  - **前端库存盘点页面**：列表页（新建盘点 Dialog 选仓库/范围/分类）+ 编辑页（实盘录入 + 盈亏差异高亮 + 差异金额计算 + 审核确认）
  - **前端库存调拨页面**：列表页（行内确认/删除操作）+ 编辑页（双仓选择 + 动态物料行增删 + 保存/确认操作）
  - 国际化：inventory/stockMovements/stockChecks/stockTransfers 四个命名空间，覆盖中/越/英三语
- **定制单管理模块（全栈）**：实现完整的定制单报价 → 确认预留 → 转销售单全流程
  - **定制单 CRUD**：后端 4 个 IPC 命令（列表筛选分页/详情/新建编辑/删除），编号自动生成 `CO-YYYYMMDD-XXX`，支持三种定制类型（尺寸/材质/全定制）和三级优先级
  - **定制单业务流转**：后端 2 个 IPC 命令（确认/取消），确认时自动根据定制 BOM 明细创建原材料预留（计算含损耗量），取消时释放预留并恢复库存 `reserved_qty`
  - **定制 BOM**：后端 1 个 IPC 命令（从标准 BOM 复制），自动关联 `custom_order_id`，复制明细和标准成本
  - **成本核算**：后端 1 个 IPC 命令，基于定制 BOM 明细 × 物料参考成本价重新计算成本
  - **转销售单**：后端 1 个 IPC 命令，从已确认定制单创建草稿销售单（带入客户/币种/报价金额），自动关联
  - **前端列表页**：5 维筛选（关键词/客户/状态/定制类型/日期范围）+ 状态操作按钮 + BusinessListTableShell 骨架
  - **前端详情/编辑页**：头信息表单 + 配置明细表格（inline 编辑）+ 定制 BOM 管理区 + 原材料预留状态展示 + 报价信息汇总（成本/加价/报价/预估毛利）
- **生产工单模块（全栈）**：实现工单全生命周期：创建 → 领料出库 → 生产中 → 完工入库 → 关闭/取消
  - **工单 CRUD**：后端创建 3 张表（`production_orders`, `production_order_materials`, `production_completions`）并提供列表/详情/新建编辑/删除基础命令，支持关联标准 BOM 或定向从定制单下发
  - **领料与退料**：后端 `pick_materials`/`return_materials` 命令，调用 `inventory_ops.rs`，支持按批次、仓库超领上限（120%）出仓。同时支持消耗定制单的特殊原材料预留库存
  - **完工入库与成本联动**：后端 `complete_production` 命令，支持多次按批完工入库（存入成品仓），完工时自动基于已领料的动态加权成本平均摊入成品库存
  - **状态流转**：前端交互与后端 `start_production`/`finish_production_order`/`cancel_production_order` 命令闭环，实现从草稿到完工各关键环节锁定保护
  - **前端列表/详情页**：新增 `/production-orders` 路由，包含列表页 5 维状态过滤、详情页/执行态 UI 隔离、独立提供领料、退料、完工入库的数据弹窗表单
  - 国际化与集成：支持中/越/英三语（`productionOrders` 命名空间），新增侧边栏 Hammer 图标导航入口
  - 国际化：customOrders 命名空间，覆盖中/越/英三语
- **智能补货模块（全栈）**：实现完整的补货策略配置 → 建议计算 → 一键采购全流程
  - **补货策略引擎**：后端 2 个 IPC 命令（`ensure_replenishment_rules` 补齐策略 / `update_replenishment_rule` 更新策略），按物料维度配置分析天数、补货周期、安全天数、批量倍数和首选供应商
  - **补货建议计算**：后端 1 个 IPC 命令（`get_replenishment_suggestions`），基于日均消耗（`sales_out` + `production_out`）× 补货周期，综合安全库存和可用库存，按批量倍数向上取整，并判定紧急度（urgent/warning/normal）
  - **消耗趋势分析**：后端 1 个 IPC 命令（`get_consumption_trend`），按日聚合 90 天出库数据，前端使用 recharts AreaChart 可视化
  - **一键生成采购单**：后端 1 个 IPC 命令（`create_purchase_orders_from_suggestions`），按供应商分组、独立事务生成草稿采购单，自动处理汇率和默认仓库映射
  - **忽略建议**：后端 1 个 IPC 命令（`ignore_suggestion`），标记不需要补货的建议为已忽略
  - **策略配置查询**：后端 1 个 IPC 命令（`get_replenishment_rules`），分页查询物料级别的策略配置
  - **前端补货看板**：建议列表（紧急度行高亮 + Badge）+ 筛选区（关键词/紧急度/分类）+ 多选批量下单（确认弹窗）+ 消耗趋势图弹窗 + 策略配置弹窗（查看/编辑/启用禁用）
  - **首页 KPI 联动**：首页看板“待补货”指标从 mock 改为实时查询补货建议数量
  - 国际化：replenishment 命名空间（含 trend/rule 子空间），覆盖中/越/英三语
- **财务管理模块（全栈）**：实现应付账款和应收账款全流程
  - **后端核心修正**：修正 `sales.rs` 中 receivables INSERT 语句字段名对齐 DDL 定义；在 `purchase.rs` 入库确认和退货确认中补充 payables 记录生成逻辑
  - **应付账款**：后端 3 个 IPC 命令（`get_payables` 含 KPI 概览 + `get_payment_records` + `record_payment`），前端列表页（KPI 卡片 + 关键字/状态筛选 + 类型/退货冲减行高亮 + 登记付款弹窗 + 付款记录查看）
  - **应收账款**：后端 3 个 IPC 命令（`get_receivables` 含 KPI 概览 + `get_receipt_records` + `record_receipt`），前端与应付对称设计
  - 金额存储使用 INTEGER 最小货币单位，避免浮点精度问题；付款/收款金额在 Rust 事务中严格校验不超过未付/未收余额
  - 国际化：finance 命名空间，覆盖中/越/英三语

---

## [0.1.3] — 2026-04-20

### 新增

- **销售管理模块（全栈）**：实现完整的销售单 → 销售出库 → 销售退货全流程，与采购模块完全对称
  - **销售单**：后端 6 个 IPC 命令（列表筛选分页/详情/新建编辑/审核/作废/删除），编号自动生成 `SO-YYYYMMDD-XXX`，多币种支持（从客户带出币种和汇率），订单级单仓模式，行折扣 + 整单折扣率叠加计算（行金额 = 数量 × 单价 × (1 - 行折扣率/100)，整单折扣金额 = 合计 × 折扣率/100），审核时信用额度检查（超限提醒不拦截）
  - **销售出库**：后端 3 个 IPC 命令（待出库明细/出库单列表/确认出库），核心事务在一个 SQLite 事务中完成：库存可用性校验（零库存强制拦截）→ 双成本快照固化（标准成本从 BOM、实际成本从库存平均成本）→ 扣减库存和批次库存 → 记录流水 → 费用分摊（中间批次四舍五入 + 最后一笔倒挤法）→ 联动销售单状态 → 生成应收账款
  - **销售退货**：后端 3 个 IPC 命令（可退明细/退货单列表/确认退货），必须关联原出库单，退货数量校验（不超过可退数量），退货成本回调（使用原出库成本快照回增库存，正确计算移动加权平均成本），批次库存回增，应收账款冲减（负数记录）
  - **前端销售单页面**：列表页（5 维筛选 + 按状态渲染操作按钮 + 出库进度显示）+ 编辑页（明细表格 inline 编辑 + 行折扣列 + 整单折扣率 + 金额汇总区 + 通用物料选择器）
  - **前端销售出库页面**：列表页（从销售单出库下拉选择器）+ 执行页（自动带出待出库明细 + 可用库存显示 + 标准成本列 + 库存不足高亮 + 一键确认出库）
  - **前端销售退货页面**：列表页（从出库单退货下拉选择器）+ 执行页（自动带出可退明细 + 退货数量校验 + 退货原因必填 + 一键确认退货）
  - 国际化：sales 命名空间 130+ 翻译键，覆盖中/越/英三语
- **采购管理模块（全栈）**：实现完整的采购单 → 采购入库 → 采购退货全流程
  - **采购单**：后端 7 个 IPC 命令（列表筛选分页/详情/新建编辑/审核/作废/删除/供应商物料快速带入），编号自动生成 `PO-YYYYMMDD-XXX`，多币种支持（从供应商带出币种和汇率），订单级单仓模式，金额自动计算（合计/折扣/运费/其他费用/订单总金额/USD 折算）
  - **采购入库**：后端 3 个 IPC 命令（待入库明细/入库单列表/确认入库），核心事务在一个 SQLite 事务中完成 7 步操作（生成单号 → 费用分摊 → 插入单据 → 更新库存 → 生成批次 → 记录流水 → 联动采购单状态），费用分摊采用中间批次四舍五入 + 最后一笔倒挤法，移动加权平均成本计算，批次追踪物料自动生成 `inventory_lots` 记录
  - **采购退货**：后端 3 个 IPC 命令（可退明细/退货单列表/确认退货），必须关联原入库单，退货数量校验（不超过可退数量），退货成本回调（重新计算移动加权平均成本），批次库存扣减
  - **库存操作基础模块**：新增 `inventory_ops.rs`，提供 9 个可复用函数（`increase_inventory` / `decrease_inventory` / `recalc_avg_cost_after_return` / `create_inventory_lot` / `decrease_lot_inventory` / `record_transaction` / `generate_lot_no` / `generate_transaction_no` / `convert_to_usd_cents`），供后续销售出库、盘点、调拨等模块复用
  - **前端采购单页面**：列表页（6 维筛选 + 按状态渲染操作按钮）+ 编辑页（明细表格 inline 编辑 + 供应商物料快速添加 + 金额汇总区）
  - **前端采购入库页面**：列表页（从采购单入库下拉选择器）+ 执行页（自动带出待入库明细 + 批次号输入 + 110% 溢量校验 + 一键确认入库）
  - **前端采购退货页面**：列表页（从入库单退货下拉选择器）+ 执行页（自动带出可退明细 + 退货数量校验 + 退货原因 + 一键确认退货）
  - 审核/作废采用原子 `UPDATE WHERE status = ?` 模式防止 TOCTOU 竞态
  - 入库确认事务中采购单状态校验在事务内执行，防止并发超量入库
  - 入库超量后端校验：单次入库数量不超过剩余可入库数量的 110%
  - 退货可退数量后端校验：退货数量不超过原入库批次剩余可退数量
  - 国际化：purchase 命名空间 100+ 翻译键，覆盖中/越/英三语
- **公共分页控件**：新增 `components/common/pagination.tsx`，提供 `PaginationControls` 组件和 `buildPageNumbers` 函数，统一所有业务列表页的分页器样式

### 优化

- **i18n 硬编码修复**：清理采购单编辑页、入库执行页、退货执行页中的中文硬编码，全部改为 `t()` 国际化调用
- **分页器代码去重**：将采购单表格、入库列表、退货列表中重复的分页器代码统一替换为公共 `PaginationControls` 组件

---

- **BOM 管理模块（全栈）**：实现完整的 BOM 物料清单管理功能
  - 后端：新增 `bom.rs` IPC 命令模块（10 个命令），支持 BOM 列表查询（关键词/状态筛选 + 分页）、详情（头 + 明细联查）、新建/更新（事务处理 + 自动重算标准成本）、删除（状态校验 + 定制单/工单引用检查）、启用/停用（同成品自动停用旧版本）、复制新版本、物料反查、需求展算（含库存缺口计算）、父项/子物料选项查询
  - BOM 编号自动生成：`BOM-YYYYMMDD-XXX` 格式
  - 标准成本核算：保存时自动汇总子物料参考进价 × 实际用量（含损耗率），USD 最小货币单位
  - 前端列表页：搜索筛选 + 表格（成品名称/编码/版本/状态/标准成本/物料数）+ 分页 + 操作菜单（编辑/复制/启用停用/删除）
  - 前端编辑页：BOM 头信息表单（成品选择/版本/生效日期）+ 明细表格（物料/用量/损耗率/工序/成本自动计算）+ 需求计算区（输入生产数量展算物料需求和库存缺口）
  - 明细编辑弹窗：物料搜索选择 + 标准用量/损耗率/工序/关键件配置
  - 物料反查组件：输入物料名称/编码查看被哪些 BOM 引用
  - 复制新版本弹窗：基于现有 BOM 快速创建新版本
  - 国际化：bom 命名空间，覆盖中/越/英三语
  - 非 Tauri 环境 mock 数据降级，Web 调试模式可用
- **仓库管理模块**：实现完整的仓库 CRUD 管理功能
  - 后端：新增 `warehouse.rs` IPC 命令模块（8 个命令），支持仓库增删改查、启用/禁用、默认仓映射管理、编码自动生成（WH-{TYPE}-{SEQ}，MAX(seq)+1 策略）
  - 删除保护：检查 8 张关联表（inventory/purchase_orders/sales_orders/inbound_orders/outbound_orders/default_warehouses/transfers/stock_checks）
  - 禁用事务一致性：禁用仓库时在同一事务中自动清除 default_warehouses 映射
  - 默认仓映射保存校验：校验仓库启用状态，禁用仓库不可设为默认仓
  - 前端页面：轻量 Card + Table 模式（数据量少不需要 BusinessListTableShell），仓库列表 + 默认仓映射两个区域
  - 编辑弹窗：使用 Field/FieldGroup 表单组件，仓库类型创建后不可修改，编码编辑模式禁用
  - 国际化：warehouses 命名空间，覆盖中/越/英三语
- **单位管理模块**：实现完整的计量单位 CRUD 管理功能
  - 后端：新增 `unit.rs` IPC 命令模块（5 个命令），支持单位增删改查、启用/禁用
  - 删除保护：检查 materials 表的 base_unit_id 和 aux_unit_id 引用
  - 前端页面：轻量 Card + Table 模式，单位列表 + 编辑弹窗
  - 编辑弹窗：使用 Field/FieldGroup 表单组件，双列布局，含小数位 Select
  - 国际化：units 命名空间，覆盖中/越/英三语
- **仓库编码共享函数**：提取 `generate_warehouse_code_internal` 为共享函数，向导（setup_create_warehouses）和管理页面共用，消除 DRY 违反
- **get_units 扩展**：物料表单下拉的 `get_units` 命令新增返回 `decimal_places` 和 `symbol` 字段，为采购模块做准备

### 优化

- **向导编码生成升级**：`setup_create_warehouses` 从 COUNT+1 改为 MAX(seq)+1 策略，避免删除后编码冲突

---

## [0.1.2] — 2026-04-17

### 新增

- **客户管理模块**：实现完整的客户档案 CRUD 管理功能
  - 后端：新增 `customer.rs` IPC 命令模块（7 个命令），支持关键词/类型/等级/国家动态筛选、分页查询、编码自动生成（CUS-YYYY-NNN）、删除保护（五表关联检查）
  - 前端列表页：关键词搜索 + 客户类型/等级/国家三维筛选 + 分页，使用 BusinessListTableShell 骨架（10 列 + sticky 首列）
  - 编辑弹窗：新增/编辑共用 Dialog，三分区表单（基本信息/结算信息/其他信息），编辑模式编码只读，前端校验（电话/邮箱格式）
  - 详情弹窗：KPI 卡片（销售总额/应收余额/已收金额）+ 联系信息条 + Tabs 切换（销售记录/应收对账）
  - 后端校验：国际电话格式、邮箱格式、枚举值、数值范围、字符串规范化（trim + 空转 NULL + email 小写）
  - 应收余额：列表页通过 LEFT JOIN receivables 实时聚合显示
  - 国际化：customers 命名空间 138 个翻译键，覆盖中/越/英三语
- **业务列表表格骨架**：新增 `BusinessListTableShell` 等公共薄封装，统一物料、供应商等业务列表的横向滚动、固定首列、分页栏、加载态和空态
  - 固定首列：统一 `BUSINESS_LIST_STICKY_HEAD_CLASS` / `BUSINESS_LIST_STICKY_CELL_CLASS`，避免各页面重复手写 sticky 样式
  - 状态行：新增 `BusinessListTableLoadingRows` / `BusinessListTableEmptyRow`，让加载态和空态保持在同一张表格结构内
  - 分页栏：新增 `BusinessListTableFooter`，统一小屏换行布局，避免分页控件独立溢出
- **供应商管理模块**：实现完整的供应商 CRUD 管理功能
  - 前端：列表筛选（关键词/国家/类别/等级）、分页、Sheet 抽屉表单（基本信息/结算/地址/银行四分区）
  - 后端：新增 `supplier.rs` IPC 命令模块（6 个命令），支持动态筛选、分页查询、编码自动生成
  - 编码规则：`SUP-YYYY-NNN` 格式，基于数据库 MAX(code) 自增避免冲突
  - 共享类型：将 `PaginatedResponse<T>` 提升为 `commands/mod.rs` 共享泛型
- **分类管理模块**：实现完整的分类树形管理功能
  - 前端：基于 react-arborist 实现交互式树形结构，支持拖拽排序、增删改查
  - 后端：新增 `category.rs` IPC 命令模块，实现分类 CRUD 和排序持久化
  - 表单验证：分类名称必填、编码唯一性校验、移动操作合法性验证
  - 交互优化：拖拽手柄独立、本地状态即时反馈、后端异步持久化

### 优化

- **业务列表小屏体验**：优化物料和供应商列表在小屏下的横向滚动与固定首列表现
  - 表格本体使用明确最小总宽度和稳定列宽，避免小屏下列被压缩却不能滚动
  - 修复主布局缺少 `min-w-0` 导致子级 `overflow-x-auto` 无法生效的问题
  - 将 sticky 首列背景改为不透明实底色，避免横向滚动时透出后方文字影子
- **代码格式化工具迁移**：从 Prettier 迁移至 Biome，统一 Rust 和 TypeScript 代码风格
  - 移除 `.prettierrc` 配置文件
  - 新增 `biome.json` 配置，统一单引号、导入排序、尾随逗号等规则
  - 排除 `next-env.d.ts` 类型声明文件

### 重构

- **分类树拖拽逻辑**：优化拖拽排序的本地状态更新与持久化逻辑
  - 重构 `handleMove` 函数，使用受控模式实现即时 UI 反馈
  - 新增 `removeNode` 和 `insertNode` 辅助函数，精确操作树节点
  - 优化后端持久化，避免不必要的 API 调用

---

## [0.1.1] — 2026-04-15

> 🚀 阶段一完成 & 阶段二启动：项目基础设施、系统设置、物料管理业务起步

### 新增

- **项目脚手架**：初始化 Next.js 16 + Tauri 2 + Tailwind CSS 4 + shadcn/ui（base-nova 风格）项目结构
- **国际化框架**：基于 next-intl 实现中/越/英三语支持，含 `login`、`changePassword`、`dashboard` 等翻译域
- **布局系统**：实现 AppLayout（可折叠侧边栏 + 顶栏 + 主内容区 + 页脚）、LocaleSwitcher 语言切换器
- **深浅主题**：CSS 变量 + next-themes，浅色蓝 / 深色暖橙双主题方案
- **首页看板**：7 个模块化子组件（KPI 指标卡、销售趋势图、库存环形图、热销 Top10、待办事项、采购趋势图、快捷操作），基于 Recharts + mock 数据
- **SQLite 数据库层**：sqlx 连接池（WAL 模式）、自管理迁移框架、45 张表 DDL + 种子数据
- **用户认证（全栈）**：
  - 前端：登录页、首次改密页、AuthProvider 路由守卫、localStorage 会话持久化
  - 后端：bcrypt 密码哈希、登录失败锁定（5 次/15 分钟）、首次登录强制改密、session_version 会话版本控制
- **IPC 通信**：`ping`、`get_db_version`、`login`、`change_password`、`get_user_info` 五个命令
- **前端工具库**：Tauri IPC 泛型封装（含非 Tauri 环境降级）、多币种格式化（VND/CNY/USD）、系统配置类型定义
- **品牌视觉**：应用图标资源（全平台）、侧边栏品牌设计
- **路由骨架**：23 个业务路由目录（PagePlaceholder 占位）
- **系统设置子页面**：完成 `appearance-content.tsx`（显示偏好设置）与 `company-info-content.tsx`（企业信息配置）的全量双向绑定开发及 UI 对齐
- **全局偏好引擎**：新增 `DisplayPreferencesProvider`，将如"紧凑列表、自动折叠侧栏、全局大字体"的前端设置项与后端 `system_config` 表及 css variables 打通
- **体验与开发流扩展**：在 `justfile` 中扩展支持生成完整 Apple HIG 等跨平台规范图标的 `icon` 快捷指令
- **看板深度多语言化**：首页仪表板中 7 个独立模块内所有写死文案全量重构至 i18n 集成翻译字典
- **首次使用向导**：完成系统首次使用向导前端集成与核心逻辑，为新用户引导基础配置
- **加载体验升级**：添加品牌闪屏（Splash Screen）加载动画组件，优化了 logo 宽度显示，提升开箱视觉体验
- **物料管理模块**：实现物料管理前端完整功能与逻辑，使用基础组件库对其表格等界面进行了全量重构对齐
- **CI/CD 流水线**：GitHub Actions 自动化 CI（ESLint + TypeScript + clippy + fmt + cargo test + 四平台构建验证），PR/push 到 main 自动触发
- **发布与安装包构建**：tag 触发的 Release 流水线，自动四平台（macOS arm64/x64、Linux x64、Windows x64）出包，含 Updater 签名与 CHANGELOG 提取；本地 `just release` 一键完成版本号同步、CHANGELOG 切版、打 tag 推送

### 优化

- **多语言管理**：将原单一庞杂的 i18n 多语言 JSON 字典文件重构为按业务模块拆分存放（如 `common`、`dashboard`、`auth`、`settings` 等）的层级目录结构。配合升级了 `request.ts` 以实现运行时的无缝合并加载，并同步更新了 `just i18n-check` 完整性校验脚本，大幅降低了后续多语言词条的维护及冲突负担。
- **系统组件层**：修复底层 `Select` 组件触发器因未自适应长度产生的多语言展示截断问题（扩展渲染宽度至 280px 级别）
- **工具导出层**：调整及修复 Tauri 工具函数中系统配置键枚举 `SystemConfigKeys` 的导出边界，确保所有前端模块稳定取用状态
- **代码依赖清理**：清除了项目中不再使用的冗余包（如 `react-hook-form`, `zod`, `zustand`, `shadcn` 等），配置了 pnpm `workspace` 化管理及版本锁定，加速后期构建处理
- **Bug 修复**：修复了配置向导默认语言选择器中缺少渲染选项的数据格式问题，并优化了页面的排版间距结构

### 重构

- 仪表盘拆分为 7 个独立子组件，提升可维护性
- 物料管理新增/编辑页面统一使用弹窗交互方式
- **系统设置模块交互重构**：
  - 规范化全部 `Select` 组件，通过 `items` 映射实现底层 API 值与前台多语言标签 (`label`) 的分离，确保数据一致性。
  - 完善外观设置页方案，新增独立的高斯模糊双拼色"跟随系统"卡片，并调整为所见即所得交互（移除保存/取消按钮）。
  - 修订全部设置页面的静态 UI 以对齐高保真原型。

### 文档

- 完成需求规格文档（12 大模块详细设计）
- 完成数据库设计文档（45 张表 DDL + ER 关系）
- 完成界面原型文档（30 个页面 wireframe + 交互规范）
- 完成开发计划文档（5 阶段任务清单）
- 多轮设计文档评审与修正（v1.0 → v1.5）
