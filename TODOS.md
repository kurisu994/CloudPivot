...
1. **多 SKU 并排比较视图（报表）**：支持同系列多个 BOM（如 1人/2人/3人沙发）并排对比，按物料+规格合并行并展示各 SKU 用量与合计。
2. **开料单子系统**：在 BOM 明细基础上，为海绵/木方/包装工序物料补充详细开料参数（长×宽×高mm、密度、部位说明等），以细化生产工艺路线。
3. **BOM 产品头信息扩展**：将产品尺寸、包装尺寸、净重/毛重、包装方式等从 Remark 抽离或与产品/物料主数据扩展字段联动。
4. **每柜用量 (TC) 自动计算**：在需求计算中支持基于标准装柜量快速计算整柜物料的合计用量，指导整柜物料采购。

**Why**: 这些功能不阻塞真实的 BOM 数据录入和目前的数据流通，作为精细化生产和报表看板的后续增强，可以降低前期的改造风险，降低主业务逻辑负担。

**Depends on / blocked by**: P0 工序步骤灵活化和 P1 分组/多语言物料上线运行稳定之后。
