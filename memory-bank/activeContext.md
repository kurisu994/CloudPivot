...
- `src-tauri/src/commands/sales.rs` — 新增 `get_sales_material_options`，为销售单选料返回真实基础单位、销售价和按仓库可用库存
- `lib/tauri/sales.ts` — 新增 `SalesMaterialOption` 类型与 `getSalesMaterialOptions` wrapper
- `app/[locale]/sales-orders/_components/sales-material-picker-dialog.tsx` — 销售单添加物料弹窗，支持搜索、待加入清单、数量/价格/折扣/备注录入
- `app/[locale]/sales-orders/_components/sales-order-edit-page.tsx` — 销售明细添加入口改为打开弹窗；移除旧 `getMaterialReferenceOptions` 选中即添加流程
- `messages/{zh,en,vi}/sales.json` — 补充销售物料弹窗和销售数量文案
- `tests/sales-material-picker.test.mjs` — 静态回归覆盖销售单不再使用旧物料 Select、销售物料选项返回基础单位/销售价/库存
- `app/[locale]/purchase-orders/_components/purchase-material-picker-dialog.tsx` — 采购单添加物料弹窗，限定当前供应商供货物料，支持搜索、待加入清单、采购数量/单价/备注录入
- `app/[locale]/purchase-orders/_components/purchase-order-edit-page.tsx` — 采购明细添加入口改为打开弹窗；移除旧供应商物料快速添加 Select
- `messages/{zh,en,vi}/purchase.json` — 补充采购物料弹窗、采购数量、交期和空状态文案
- `tests/purchase-material-picker.test.mjs` — 静态回归覆盖采购单不再使用旧快速添加 Select，且弹窗候选仍来自供应商供货物料
